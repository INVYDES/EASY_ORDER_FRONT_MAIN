  ...
  <div class="space-y-6 px-4 sm:px-6">
    ...
    <div class="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1 overflow-x-auto">
      <button v-for="t in mainTabs" :key="t.key" @click="mainTab = t.key"
        :class="['px-3 sm:px-5 py-2 text-[11px] sm:text-sm font-medium rounded-lg transition whitespace-nowrap',
    ...
    <template v-if="mainTab === 'resumen'">
      <LoadingSpinner v-if="loading.general" />
      ...
      <template v-else>
        ...
        <div class="border-b border-gray-200 dark:border-gray-700 mb-6">
          <nav class="-mb-px flex gap-3 sm:gap-6 overflow-x-auto">
            <button v-for="tab in crudTabs" :key="tab.key" @click="activeTab = tab.key"
              :class="['py-3 sm:py-4 px-1 border-b-2 font-medium text-[11px] sm:text-sm transition whitespace-nowrap',
              ...
              {{ tab.label }}
              <span v-if="tab.key !== 'sesiones'" class="ml-1.5 bg-gray-100 text-gray-600 rounded-full px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs font-semibold dark:bg-gray-700 dark:text-gray-300">{{ tab.count }}</span>
            </button>
          </nav>
        </div>
        ...
        <div v-if="activeTab === 'sesiones'" class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 animate-fade-in dark:bg-gray-800 dark:border-gray-700">
          ...
          <LoadingSpinner v-if="loading.general" text="Cargando historial de entradas y salidas..." />
          <div v-else class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700">
              <thead class="bg-gray-50 dark:bg-gray-800/50">
                <tr>
