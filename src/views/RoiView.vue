<template>
  <div class="space-y-5">
    ...
    <!-- ── Selector de período ────────────────────────────────────────────── -->
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 flex flex-wrap items-center gap-3">
      ...
      <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
        <input 
          v-model="fechaInicio" 
          type="date"
          class="px-3 py-1.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none" 
        />
        <span>→</span>
        <input 
          v-model="fechaFin" 
    ...
    <div v-if="loading" class="flex items-center justify-center py-16 gap-3">
      ...
      <p class="text-gray-400 dark:text-gray-500 text-sm">Calculando ROI...</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="font-semibold text-gray-800 dark:text-gray-200">📊 ROI por ingredientes</h3>
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Margen real = Ventas − Costo de recetas</p>
          </div>
          <div class="flex items-center gap-2">
            ...
            <button 
              ...
              class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-500 text-xs font-semibold"
            >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
          <div class="bg-indigo-50 dark:bg-indigo-900/30 rounded-xl p-4 text-center">
            <p class="text-xs text-indigo-600 dark:text-indigo-400 font-semibold uppercase">Ventas</p>
            <p class="text-xl font-black text-indigo-700 dark:text-indigo-400">${{ fm(roiIngredientes.ventas) }}</p>
            <p class="text-[10px] text-indigo-500 dark:text-indigo-400 mt-1">{{ productosRoi.length }} productos vendidos</p>
          </div>
          <div class="bg-red-50 dark:bg-red-900/30 rounded-xl p-4 text-center">
            <p class="text-xs text-red-600 dark:text-red-400 font-semibold uppercase">Costo recetas</p>
            <p class="text-xl font-black text-red-700 dark:text-red-400">${{ fm(roiIngredientes.costo) }}</p>
            <p v-if="!roiIngredientes.costo" class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Sin ingredientes asignados</p>
            <p v-else class="text-[10px] text-red-500 dark:text-red-400 mt-1">{{ roiIngredientes.costoPorcentaje }}% del total</p>
          </div>
        ...
        <div class="relative h-56">
          <div v-if="!productosRoi.length" class="absolute inset-0 flex flex-col items-center justify-center text-gray-400 dark:text-gray-500">
        ...
        <div v-if="productosRoi.length" class="mt-4 space-y-1.5 max-h-52 overflow-y-auto">
          <div v-for="p in productosRoi" :key="p.nombre" class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700/50">
            <div class="flex-1 min-w-0">
              <p class="text-xs font-semibold text-gray-800 dark:text-gray-200 truncate">{{ p.nombre }}</p>
              <p class="text-[10px] text-gray-400 dark:text-gray-500">{{ p.total_vendido }} uds · Venta ${{ fm(p.venta_total) }} · Costo ${{ fm(p.costo_total) }}</p>
            </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="font-semibold text-gray-800 dark:text-gray-200">💼 ROI operativo</h3>
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Ventas vs gastos registrados (renta, nómina, servicios...)</p>
          </div>
          <div class="flex items-center gap-2">
        ...
        <div v-if="!gastoResumen.total_gastos" class="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">
        ...
        <template v-else>
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <div class="bg-indigo-50 dark:bg-indigo-900/30 rounded-xl p-4 text-center">
              <p class="text-xs text-indigo-600 dark:text-indigo-400 font-semibold uppercase">Ventas</p>
              <p class="text-xl font-black text-indigo-700 dark:text-indigo-400">${{ fm(gastoResumen.ventas) }}</p>
              <p class="text-[10px] text-indigo-500 dark:text-indigo-400 mt-1">Total período</p>
            </div>
            <div class="bg-red-50 dark:bg-red-900/30 rounded-xl p-4 text-center">
              <p class="text-xs text-red-600 dark:text-red-400 font-semibold uppercase">Gastos</p>
              <p class="text-xl font-black text-red-700 dark:text-red-400">${{ fm(gastoResumen.total_gastos) }}</p>
              <p class="text-[10px] text-red-500 dark:text-red-400 mt-1">{{ gastoResumen.por_categoria ? Object.keys(gastoResumen.por_categoria).length : 0 }} categorías</p>
            </div>
            <div class="rounded-xl p-4 text-center" :class="gastoResumen.utilidad_bruta >= 0 ? 'bg-emerald-50 dark:bg-emerald-900/30' : 'bg-rose-50 dark:bg-rose-900/30'">
              <p class="text-xs font-semibold uppercase" :class="gastoResumen.utilidad_bruta >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">
                Utilidad
              </p>
              <p class="text-xl font-black" :class="gastoResumen.utilidad_bruta >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400'">
                ${{ fm(gastoResumen.utilidad_bruta) }}
              </p>
              <p class="text-[10px] mt-1" :class="gastoResumen.utilidad_bruta >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">
          ...
          <div class="flex items-center gap-6 flex-wrap">
            <div class="relative h-40 w-40 shrink-0">
              ...
              <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <p class="text-sm font-black text-gray-800 dark:text-gray-200">${{ fm(gastoResumen.total_gastos) }}</p>
                <p class="text-[10px] text-gray-400 dark:text-gray-500">total</p>
              </div>
            </div>
            <div class="flex-1 space-y-2">
              <div v-for="(monto, cat) in gastoResumen.por_categoria" :key="cat" class="flex items-center gap-2">
                ...
                <span class="text-xs text-gray-600 dark:text-gray-400 flex-1 capitalize font-medium">{{ cat }}</span>
                <span class="text-xs font-bold text-gray-800 dark:text-gray-200">${{ fm(monto) }}</span>
                <span class="text-[10px] text-gray-400 dark:text-gray-500 w-10 text-right">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="font-semibold text-gray-800 dark:text-gray-200">📅 Comparación vs período anterior</h3>
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Ventas, pedidos y ticket promedio</p>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div v-for="item in comparacion" :key="item.label" class="bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
            <p class="text-xs text-gray-500 dark:text-gray-400 font-semibold uppercase mb-2">{{ item.label }}</p>
            <p class="text-xl font-black text-gray-900 dark:text-gray-100">{{ item.actual }}</p>
            ...
            <p class="text-[10px] text-gray-400 dark:text-gray-500 mt-1">Anterior: {{ item.anterior }}</p>
            ...
            <div class="flex gap-1 h-2 mt-3">
              ...
              <div class="bg-gray-200 dark:bg-gray-600 rounded-full transition-all" :style="{ width: item.pctAnterior + '%' }"></div>
            </div>
          </div>
        </div>
      ...
      <!-- ── Recomendaciones ──────────────────────────────────────────────── -->
      <div v-if="recomendaciones.length > 0" class="bg-gradient-to-r from-amber-50 dark:from-amber-900/30 to-orange-50 dark:to-orange-900/30 rounded-xl p-5 border border-amber-200 dark:border-amber-700">
        ...
        <div class="space-y-2">
          <div v-for="(rec, idx) in recomendaciones" :key="idx" class="flex items-start gap-2 text-sm text-amber-700 dark:text-amber-400">
    ...
    <div v-if="showModalGasto" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4 animate-fade-in" @click.self="showModalGasto = false">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6 animate-slide-up">
        <div class="flex items-center justify-between mb-5">
          <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">
            <i class="fa-solid fa-receipt text-indigo-600 dark:text-indigo-400 mr-2"></i>
            Registrar gasto
          </h3>
          <button @click="showModalGasto = false" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 text-xl transition">✕</button>
        </div>
        
        <div v-if="gastoError" class="mb-4 p-3 text-sm text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl flex items-start gap-2">
          <i class="fa-solid fa-exclamation-triangle text-red-500 dark:text-red-400 mt-0.5"></i>
        ...
        <form @submit.prevent="guardarGasto">
          <div class="space-y-4">
            <div>
              ...
              <input 
                ...
                class="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none transition"
                required
              />
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                ...
                <select 
                  ...
                  class="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none bg-white dark:bg-gray-700"
                  required
                >
              ...
              <div>
                ...
                <div class="relative">
                  <span class="absolute left-3 top-3 text-gray-400 dark:text-gray-500 text-sm">$</span>
                  <input 
                    ...
                    step="0.01"
                    placeholder="0.00"
                    class="w-full pl-7 pr-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none"
                    required
                  />
                </div>
              </div>
            ...
            <div>
              ...
              <input 
                ...
                type="date"
                class="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea 
                ...
                class="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 focus:outline-none resize-none"
              ></textarea>
            </div>
          </div>
          ...
          <div class="flex gap-3 mt-6">
            <button 
              ...
              class="flex-1 py-2.5 text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition font-medium"
            >
              Cancelar
            </button>
            <button 
    ...
    <div v-if="showDetalleIngredientes" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4 animate-fade-in" @click.self="showDetalleIngredientes = false">
      <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[80vh] overflow-y-auto">
        <div class="sticky top-0 bg-white dark:bg-gray-800 z-10 flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700">
          <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">
            <i class="fa-solid fa-chart-line text-indigo-600 dark:text-indigo-400 mr-2"></i>
            Detalle de ROI por producto
          </h3>
          <button @click="showDetalleIngredientes = false" class="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 transition text-xl">✕</button>
        </div>
        <div class="p-5">
          <div class="space-y-3">
            <div v-for="p in productosRoi" :key="p.nombre" class="border border-gray-100 dark:border-gray-700 rounded-xl p-4 hover:shadow-sm transition">
              <div class="flex items-center justify-between mb-2">
                <h4 class="font-bold text-gray-800 dark:text-gray-200">{{ p.nombre }}</h4>
              ...
              <div class="grid grid-cols-3 gap-3 text-center text-xs">
                <div>
                  <p class="text-gray-500 dark:text-gray-400">Vendidos</p>
                  <p class="font-bold text-gray-800 dark:text-gray-200">{{ p.total_vendido }} uds</p>
                </div>
                <div>
                  <p class="text-gray-500 dark:text-gray-400">Venta total</p>
                  <p class="font-bold text-indigo-600 dark:text-indigo-400">${{ fm(p.venta_total) }}</p>
                </div>
                <div>
                  <p class="text-gray-500 dark:text-gray-400">Costo total</p>
                  <p class="font-bold text-red-600 dark:text-red-400">${{ fm(p.costo_total) }}</p>
                </div>
              </div>
              <div class="mt-3">
