<template>
  <div class="space-y-5 p-4 sm:p-6">
    ...
    <div v-if="productoEliminar" class="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center px-4 backdrop-blur-sm animate-fade-in"
      @click.self="productoEliminar = null">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-scale-up dark:bg-gray-800">
        <div class="p-8 text-center">
          <div class="w-16 h-16 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 rounded-full flex items-center justify-center mx-auto mb-6 text-2xl">
            🗑️
          </div>
          <h3 class="text-xl font-bold text-gray-900 mb-2 dark:text-gray-100">¿Eliminar producto?</h3>
          <p class="text-gray-500 text-sm leading-relaxed mb-6 dark:text-gray-400">
            Estás a punto de eliminar <span class="font-bold text-gray-800 dark:text-gray-200">{{ productoEliminar.nombre }}</span>.
            Esta acción es definitiva y no se puede deshacer.
          </p>

          <div class="flex flex-col gap-3">
            <button @click="confirmarEliminarProducto" :disabled="loading.products"
              ...
              {{ loading.products ? 'Eliminando...' : 'Sí, Eliminar' }}
            </button>
            <button @click="productoEliminar = null"
...
const handleDelete = (id) => {
  ...
  productoEliminar.value = { id, nombre: prod?.nombre || `ID ${id}` }
}

const confirmarEliminarProducto = async () => {
  if (!productoEliminar.value) return
  try {
    await apiClient.delete(`/productos/${productoEliminar.value.id}`)
