  ...
  try {
    ...
    modalIngredientes.value.items = resultados
      ...
      .map(({ detalle, ingredientes }) => ({
        ...
        producto_nombre: detalle.producto_nombre ?? (detalle.tamano_nombre ? `${detalle.producto?.nombre || 'Producto'} (${detalle.tamano_nombre})` : (detalle.producto?.nombre ?? `Producto #${detalle.producto_id}`)),
