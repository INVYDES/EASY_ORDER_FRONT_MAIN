      ...
      <div class="bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div class="px-5 py-4 border-t border-gray-800 flex items-center gap-3">
          ...
          <button v-if="modalIngredientes.nuevoEstado"
            ...
            class="flex-1 py-3 text-base font-bold text-white bg-blue-600 hover:bg-blue-500 rounded-xl transition disabled:opacity-50">
  ...
  try {
    ...
    modalIngredientes.value.items = resultados
      ...
      .map(({ detalle, ingredientes }) => ({
        ...
        producto_nombre: detalle.producto_nombre ?? (detalle.tamano_nombre ? `${detalle.producto?.nombre || 'Bebida'} (${detalle.tamano_nombre})` : (detalle.producto?.nombre ?? `Bebida #${detalle.producto_id}`)),
