<template>
  ...
  >
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-100">
      ...
      <div class="px-6 py-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
        ...
        <button @click="$emit('close')" class="p-2 hover:bg-white dark:bg-gray-800 rounded-full text-slate-400 hover:text-slate-600 transition-colors shadow-sm border border-transparent hover:border-slate-100">
      ...
      <div class="p-6">
        ...
        <div class="space-y-5">
          <div>
            ...
            <input v-model="form.nombre" type="text" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:bg-gray-800 focus:outline-none transition-all" />
          </div>

          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Unidad *</label>
              <select v-model="form.unidad" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:bg-gray-800 focus:outline-none transition-all appearance-none cursor-pointer">
            ...
            <div>
              ...
              <div class="relative">
                ...
                <input v-model="form.costo_unitario" type="number" step="0.01" class="w-full pl-8 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:bg-gray-800 focus:outline-none transition-all" />
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <input v-model="form.proveedor" type="text" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500 focus:bg-white dark:bg-gray-800 focus:outline-none transition-all" />
          </div>
        </div>
      </div>

      <div class="px-6 py-5 bg-slate-50/50 border-t border-slate-100 flex gap-3">
        <button @click="$emit('close')" class="flex-1 py-2.5 text-sm font-bold text-slate-500 bg-white dark:bg-gray-800 border border-slate-200 rounded-xl hover:bg-slate-50 transition active:scale-95">
          Cancelar
        </button>
