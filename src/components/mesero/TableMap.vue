  ...
  <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-5">
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-3 text-xs flex-wrap">
        ...
        <span class="flex items-center gap-1 text-gray-500 dark:text-slate-400"><span class="w-3 h-3 rounded-full bg-rose-200 dark:bg-rose-800/60 border border-rose-400 dark:border-rose-600 animate-pulse inline-block"></span> Inactiva</span>
      </div>
    </div>
    ...
    <div class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-8 gap-3">
      ...
      >
        ...
        <span v-else-if="mesa.ocupada && mesa.needsAttention" class="text-[10px] font-bold text-rose-600 dark:text-rose-400 mt-0.5 flex items-center gap-1">
...
const mesas = computed(() =>
  Array.from({ length: props.totalMesas }, (_, i) => {
    ...
    const inactividadMinutos = ordenMesa ? Math.floor((Date.now() - new Date(ordenMesa.updated_at || ordenMesa.created_at).getTime()) / 60000) : 0
    const needsAttention = ordenMesa ? inactividadMinutos >= MINUTOS_INACTIVIDAD : false
    return {
      numero: i + 1,
...
const estiloMesa = (mesa) => {
  ...
  if (mesa.ocupada && mesa.needsAttention) return 'bg-rose-50 dark:bg-rose-900/30 border-rose-300 dark:border-rose-700 hover:bg-rose-100 dark:hover:bg-rose-900/50 hover:border-rose-400 animate-pulse'
...
const textoMesa = (mesa) => {
  ...
  if (mesa.ocupada && mesa.needsAttention) return 'text-rose-700 dark:text-rose-400'
