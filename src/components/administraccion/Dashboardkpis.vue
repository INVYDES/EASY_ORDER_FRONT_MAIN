<template>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div v-for="card in cards" :key="card.label"
      class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-5 border-l-4 relative overflow-hidden"
      :class="card.border">
      <!-- Icono fondo decorativo -->
      <span class="absolute right-3 top-2 text-4xl opacity-10 select-none">{{ card.icon }}</span>
      <p class="text-xs font-semibold text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wide">{{ card.label }}</p>
      <p class="text-3xl font-black text-gray-900 dark:text-gray-100 mt-1 leading-none">{{ card.value }}</p>
      <div v-if="card.sub" class="flex items-center gap-1 mt-2">
        <span class="text-xs" :class="card.subColor || 'text-gray-400 dark:text-gray-500'">{{ card.sub }}</span>
      </div>
    </div>
  </div>
</template>
